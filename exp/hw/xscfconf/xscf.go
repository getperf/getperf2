...
func (e *Xscf) RunRemoteServer(ctx context.Context, env *cfg.RunEnv) error {
...
func (e *Xscf) Run(ctx context.Context, env *cfg.RunEnv) error {
	startTime := time.Now()
	errFile, err := env.OpenLog("error.log")
	if err != nil {
		return errors.Wrap(err, "prepare Xscf inventory error")
	}
	defer errFile.Close()
	e.errFile = errFile
	e.Env = env
	...
	log.Infof("Complete Xscf inventory collection %s", msg)
